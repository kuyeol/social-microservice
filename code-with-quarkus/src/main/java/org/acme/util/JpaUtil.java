...
public class JpaUtil
{
        ...
        public static String toOrderBy(JpaSort sort) {
            if (sort == null) {
                return null;
            }
            ...
            for (int i = 0; i < sort.getColumns().size(); i++) {
                JpaSort.Column column = sort.getColumns().get(i);
                if (i > 0)
                    sb.append(" , ");
                ...
                if (column.getDirection() != JpaSort.Direction.Ascending) {
                    sb.append(" DESC");
                }

                if (column.getNullPrecedence() != null) {
                    if (column.getNullPrecedence() == JpaSort.NullPrecedence.NULLS_FIRST) {
        ...
        private static String unquoteColumnName(String columnName) {
            ...
            if (unquotedColumnName.indexOf('`') >= 0) {
                throw new QueryException("JpaSort column name cannot have backticks");
            }
